...
class StoryMenuState extends MusicBeatState
{
	...
	public var weekData:Array<Array<String>> = [];
	...
	override function create()
	{
		...
		for (i => week in WeekRegistry.weekList)
		{
			...
			if (parsedWeek == null || parsedWeek.storymodeVisible == false) continue;
		...
		scoreText = new FunkinText(10, 10, 0, "SCORE: 49324858", 36);
		scoreText.setFormat("VCR OSD Mono", 32);

		txtWeekTitle = new FunkinText(FlxG.width * 0.7, 10, 0, "", 32);
		txtWeekTitle.setFormat("VCR OSD Mono", 32, FlxColor.WHITE, RIGHT);
		txtWeekTitle.alpha = 0.7;

		var rankText:FunkinText = new FunkinText(0, 10);
		...
		txtTracklist = new FunkinText(FlxG.width * 0.05, yellowBG.x + yellowBG.height + 100, 0, "Tracks", 32);
	...
	function selectWeek()
	{
		...
		if (!stopspamming)
		{
			...
			for (character in grpWeekCharacters.members)
				if (character.animation.getNameList().contains('confirm')) character.animation.play('confirm');
			stopspamming = true;
		}
	...
	function updateText()
	{
		...
		for (i => char in weekCharacters[curWeek])
		{
			...
			if (char == null || char == '' || addedOldChar) continue;
